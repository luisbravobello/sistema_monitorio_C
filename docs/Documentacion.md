...
| RF-04 | El sistema debe detectar eventos de tipo `PORT_SCAN` cuando una misma IP acceda a 4 o más puertos distintos en una ventana de 15 segundos. |
| RF-05 | El sistema debe detectar eventos de tipo `SSH_BRUTE` cuando una misma IP realice 3 o más conexiones al puerto 22 en una ventana de 15 segundos. |
...
|--------|------|-------------|
| `POST` | `/start?device=X` | Inicia la captura de paquetes en la interfaz `X`. Apertura síncrona: si Npcap/libpcap no está disponible, falta el permiso de Administrador/root o el dispositivo no existe, la respuesta trae el error real. Si no se especifica `device`, se autoselecciona la primera interfaz real disponible. |
...
| `GET` | `/search?ip=&port=&timestamp=` | Filtra eventos por IP de origen y/o puerto (búsqueda lineal), o busca por timestamp exacto (búsqueda binaria, requiere el arreglo ya ordenado) |
| `GET` | `/devices` | Devuelve las interfaces reales detectadas por Npcap/libpcap y un flag `real_capture` (true/false) que indica si el binario corre con captura real o con el simulador |
| `POST` | `/clear` | Limpia todos los eventos y alertas almacenados en memoria |
| `POST` | `/simulate?type=` | Genera una alerta de prueba (`ssh_brute` o `port_scan`, por defecto `ssh_brute`) sin necesidad de tráfico real ni permisos de captura |
...
El detector (`detector.c`) usa una **ventana deslizante de 15 segundos** sobre un historial circular de 256 entradas:
...
──────────────────      ─────────────────────────      ─────────────────────────
 ...
 Simulador interno   ┘         (crudo)         (ventana de 15 s)      (si hay amenaza)
...
4. **Análisis:** cada `NetworkPacket` se envía a `detector_analyze`, que evalúa la ventana de tiempo de 15 s para la IP correspondiente.
...
- **Estructuras de tamaño fijo:** el historial del detector (256 entradas, `HISTORY_SIZE`), el arreglo de eventos (5000 entradas, `MAX_EVENTS`) y el de alertas (1000 entradas, `MAX_ALERTS`) son estáticos, lo que simplifica la gestión de memoria y la sincronización, a costa de un límite de capacidad.
